...
　　考虑计算一个$must_i$表示第$i$元素必选时全局的最优解。假设我们得到了这个数组，在询问$a_x=y$时，由于$a_x$在原来的$must_x$的贡献为$-a_x$，我们只需要简单地将那个贡献$-a_x$消除，再加上新的贡献$-y$即可。可以证明，当我们钦定的元素的值改变时，其他地方的决策也不会变；否则如果改动后存在更优的方案，那么那个方案在未修改之前应该就是$must_x$
...
​	　　注意到这个形式与之前的DP其实是一样的，只不过我们钦定了一些初值，使得这个形式可以用于强制选择的转移

　　注意到$t_r$可以更新$must_{s_m+1...s_r}$，因此我们对$t$做一个后缀$\max$，再更新到$must$上。

　　为什么$must_{s_l...s_m}$不能更新呢？因为在这个DP中，我们只能对于每一个$t_i$保证这个值体现的最优的$[l,r]$的$r$一定在$i$或其右边，却无法得知相应的$l$具体在什么地方。由此，在每一次分治时，左半边是无法得到更新的。但注意到这个问题具有良好的对称性，我们将整个序列翻转以后反过来再做一次分治，即可更新现在没更新到的左半边（注意我们要保证两次分治的分治中点也要对称，因此对于区间长度为奇数的情况，中点要偏移一位）



# Summary

　　对于规模较大的DP，设置二维DP，记录每一步的过程（如在当前段进行了多少）并用数据结构省略第一维并加速转移固然是一种方法，但是如果在这个思路上碰到了一些很难或无法维护的东西而导致无法快速转移，不要继续死磕。尝试省掉第二维，考虑整段转移，其式子看似玄乎，但有可能可以使用斜率优化加速。
