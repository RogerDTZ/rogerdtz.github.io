...
* 跨过了某一座桥：选这座桥
* 跨过了两座桥：任选一个
* 在两座桥的正中间：这个是重点，记两个桥的坐标分别为$x$和$y$，则我们要拿$\mid x-a\mid+\mid x-b\mid$和$\mid y-a \mid + \mid y-b \mid$比较，谁小选谁；可每一个人都是两个值，不好量化。我们可以取$a$与$b$的平均值$m=a+b$，则其实要拿的是$2\left |m-x\right | $与$2\left | m-y\right | $比较。
